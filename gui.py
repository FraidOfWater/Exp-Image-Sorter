...
#destination windows take focus and show next fails
#index is reset or animation is started again on any keypress.
#img may crash if too many, make the drop out of memory like webm and gif already.
#destination view is a bit finicky
#do navigator, navigate by key
#scroll can be impeded by loading of the zoomed image
#change all isanimated and other stuff to "static" "animated" "video" no inbetweens or difficulty.
#First launch second_window_viewer doesn't work.
#Videos broken on second_viewer. doesnt scale
#Resizing for all others too beside video.
#mp4 not pixel perfect in viewer
...
class Imagefile:
    ...
    def __init__(self, name, path) -> None:
        ...
        self.isvisibleindestination = False # prevents two animation loop at same time when gif visible on dest and main
        ...
        #need width and height, file_type (can it be animated, as well, so framecount 0 = static) remove default delay
        #static
        #animated - gif, webp, webm (can be displayed in grid)
        #video - webm and mp4 (mp4 only displayed in viewer, webm displayed using vlc.)
        #must use vlc to display webm in grid?
...
class SortImages:
        ...
        if os.path.exists(sessionpath) and os.path.isfile(sessionpath):
            ...
            for line in savedata['imagelist']:
                if os.path.exists(line['path']):
                    ...
                    if obj.isanimated:
                        try:
                            obj.dimensions=line['dimensions']
                        except Exception as e:
                            print(f"No value dimensions: {e}")
    ...
    def setDestination(self, *args):
        current_time = time()
        if not self.navigator.dest_keys_pressed:
            pass
        ...
        current_list = self.get_current_list() # Current list being compared
        try:
            wid = args[1].widget
        ...
        # Return all images whose checkbox is checked (And currently in view by image viewer, so you can just press a hotkey and not have to check a checkbox everytime) (If interacting with other squares, it will cancel itself out. This is so user wont accidentally move anything.)
            ...
            marked = [x for x in current_list if x.obj.checked.get()] # All checked are now in marked list.

            "Current selection is included"
            if self.navigator.old and self.gui.focused_on_secondwindow: # to see if we have clicked elsewhere as to not move the displayed image anymore.
                for x in current_list:
                    if self.navigator.old.obj.id == x.obj.id:
                        if x not in marked:
                            marked.append(x)
        ...
        if hasattr(self.gui, 'destwindow'): #only refresh dest list if destwindow active.
            self.gui.refresh_destinations()
        #Navigator show_next
        if self.gui.show_next.get() and len(self.gui.displayedlist) >= 1 and hasattr(self.gui, "Image_frame"):
            self.navigator.select_next(self.gui.displayedlist)
    ...
    def makethumb(self, imagefile):
        ...
        # If any, find thumbnails from data folder and save the path to imagefile
        if (os.path.exists(thumbpath)): # The exists, just point to it.
    ...
    def generate_thumbframes(self, anims):
        #4: 12.16-13.07 : very scrollable
        #5: 11.76 : scrollable
        def newmethod():
            with concurrent.ThreadPoolExecutor(max_workers=max_workers) as executor1:
                executor1.map(self.load_thumb_frames, anims)
            print(f"Thumbframes generated in: {perf_counter()-a:.2f}")
        a = perf_counter()
        max_workers = max(1,self.threads)
        Thread(target=newmethod, daemon=True).start()
        

    #@profile
    def empty_thumb_frames(self, gridsquare):
        gridsquare.obj.index = 0
        gridsquare.canvas.itemconfig(gridsquare.canvas_image_id, image=gridsquare.canvas.image)
        gridsquare.obj.frames = []
        gridsquare.obj.lazy_loading = True
        
    
    #thread this too
    def load_thumb_frames_again(self, anims):
        def newmethod():
            with concurrent.ThreadPoolExecutor(max_workers=max_workers) as executor1:
                executor1.map(newmethod2, anims)
            print(f"Thumbframes generated in: {perf_counter()-a:.2f}")
            for x in anims:
                print(x.name.get(), len(x.frames))
        def newmethod2(obj):
            ...
            if obj.path.lower().endswith(".webm"):
                ...
                #print("done for webm", obj.name.get(), self.timer.stop())
                return
            ...
            # Load frames for GIF, WEBP
                ...
                try:
                    ...
                    with Image.open(obj.path) as img:
                        ...
                        #print("done for gif, webp", obj.name.get(), self.timer.stop())
                except Exception as e: #fallback to static.
                    logger.error(f"Error in load_thumb_frames_again: {e}")
    ...
    def load_thumb_frames(self, obj): # Creates frames and frametimes for gifs and webps
        ...
        # Load frames for GIF, WEBP
            ...
            try:
                ...
                with Image.open(obj.path) as img:
                    ...
                    logger.debug(f"Found animated: {obj.name.get()[:30]} with {obj.framecount} frames.")
                ...
                logger.error(f"Error in load_thumb_frames: {e}")
                obj.isanimated = False
