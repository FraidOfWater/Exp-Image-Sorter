...
class Destination_Viewer():
    def __init__(self, fileManager):
        ...
        style = ttk.Style()
        style.configure("Theme_square.TCheckbutton", background=self.gui.grid_bg, foreground=self.gui.button_text)
        self.displayedlist = []
    ...
    def add_squares(self, imagetuples: list) -> None:
        ...
        regen = []
        for gridsquare in imagetuples:
            square, command = gridsquare
            if command == "add":
                square = self.makedestsquare(square)
                self.destgrid.window_create(
                "1.0", window=square, padx=self.gui.square_padx, pady=self.gui.square_pady)
                self.displayedlist.append(square)
                self.displayedlist_ids.append(square.obj.id)
                self.displayedset.add(square)
                
            elif command == "refresh":
                self.destgrid.window_configure(square, window="")
                self.displayedlist.remove(square)
                self.displayedlist_ids.remove(square.obj.id)
                self.destgrid.window_create(
                "1.0", window=square, padx=self.gui.square_padx, pady=self.gui.square_pady)
                self.displayedlist.append(square)
                self.displayedlist_ids.append(square.obj.id)
            
            if square.obj.thumb == None: # Checks if img is unloaded
                regen.append(square.obj)
...
#
            ...
            check_frame = tk.Frame(frame, height=self.gui.textbox_size, padx=2)
...
#
    ...
    def close_window(self, *args, event=False, reopenargs=None, reopen=False):
        ...
        self.window_is_closing = True ### need the exitin flag or joins?
        ...
        if fileManager.navigator.old in self.displayedset:
            fileManager.navigator.old = None
