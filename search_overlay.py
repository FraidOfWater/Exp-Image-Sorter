...
class ImageViewer:
    ...
    def on_key_press(self, e):
        ...
        if not self.include_folder or e.keysym == "Control_L": return
        ...
        if (not self.search_active and e.keysym in ("space", "Tab")): # or (not self.search_active and not self.hotkey_active and len(e.char) == 1 and e.char.isprintable()):
            ...
            if e.keysym == "Delete": pass# send to trash
            elif e.keysym == "Return": pass # Autosort
            else:
            ...
            if self.search_minimized and ((len(e.char) == 1 and e.char.isprintable()) or e.keysym in ("Return", "BackSpace", "Up", "Down", "Tab", "space")): # behavior keypresses reset, space resets, nav unminimizes, backspace unminimizes
                ...
                if e.keysym == "BackSpace": pass
                elif e.keysym in ("Up", "Down"): self.navigate(e.keysym)
                ...
                elif e.keysym == "Escape": self.close_search()
                elif e.keysym == "BackSpace":
                    if self.search_text == "": self.close_search()
                    else:
                        self.search_text = self.search_text[:-1]
                        self.update_search()
                elif e.keysym == "Delete":
                    if self.search_minimized:  return
    ...
    def update_search(self):
        whole_query = self.search_text.lower()
        if not whole_query: # Empty search
        ...
        plain_queries = whole_query.split(" ")
        path_queries = []
        word_queries = []
        ...
        filtered_cache = [] # holds the queries that MATCH.

        for dirname, rel_path in self.cached_dirs:
            rel_path = rel_path.replace("/", "\\")
            match = True # both queries must pass

            if not all(path_query in rel_path.lower() for path_query in path_queries): match = False # all path_queries must be found in the path.
            if not all(word_query in dirname.lower() for word_query in word_queries): match = False # all word_queries must be found in the dirname.
            if match: filtered_cache.append((dirname, rel_path))

        # Path Depth, Name Length, Alphabetical
        filtered_cache.sort(key=lambda x: (len(x[1].split("\\")), len(x[0]), x[0]))

        recent_paths = [r.replace("/", "\\").lower() for n, r in self.recent_searches]
        matching_recents = [item for item in filtered_cache if item[1].lower() in recent_paths]
        ...
        elif self.selected_index >= len(self.search_results): self.selected_index = 0

        self.update_search_box()

    def navigate(self, direction):
        if not self.search_results: return
        if direction == "Down": self.selected_index = min(self.selected_index + 1, len(self.search_results) - 1)
        else: self.selected_index = max(self.selected_index - 1, 0)
        if self.selected_index >= self.scroll_offset + self.max_visible: self.scroll_offset += 1
        elif self.selected_index < self.scroll_offset: self.scroll_offset -= 1
        self.update_search_box()
    ...
    def update_search_box(self):
        ...
        if self.search_minimized:
            ...
            self.canvas.coords(self.search_ui["text"], x1+10, y1+15)
        ...
        if "results" not in self.search_ui: self.search_ui["results"] = []
        ...
        for i in range(self.max_visible):
            ...
            if result_idx < len(self.search_results) and i < visible_count:
                ...
                self.canvas.itemconfig(name_id, text=name, fill=fg, font=("Arial", 12, "bold"), state='normal')

                self.canvas.coords(rel_id, x1 + 15, y + 18)
                self.canvas.itemconfig(rel_id, text=rel, fill="#888", font=("Arial", 9), state='normal')
                y += 34
            else:
    ...
    def create_hotkey_box(self):
        ...
        title = self.canvas.create_text(x + 10, y + 10, anchor="nw", tags="sorter", text="Hotkeys", fill="white", font=("Arial", 14, "bold"))
        items = []
        y_offset = 35
        for k, v in hotkeys:
            t = self.canvas.create_text(x + 10, y + y_offset, anchor="nw", tags="sorter", text=f"{k}: {os.path.basename(v[1])}", fill="white", font=("Arial", 12))
            items.append(t)
            y_offset += 24
    ...
    def show_hotkeys(self, event=None):
        ...
        if self.search_active: self.close_search()

    def close_hotkeys(self):
        if not self.hotkey_active: return
